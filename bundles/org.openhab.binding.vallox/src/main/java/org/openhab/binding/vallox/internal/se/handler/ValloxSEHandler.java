    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        if (this.thing.getStatus() == ThingStatus.ONLINE && isConnected()) {
            ...
            if (command instanceof RefreshType) {
                ...
                QuantityType<?> localCommand = ((QuantityType<?>) command).toUnit(descriptor.getUnit());
                if (localCommand != null) {
                    handleDecimalCommand(localCommand.intValue(), descriptor);
                } else {
                    logger.debug("Unit conversion for channel {} failed", descriptor.channelID);
                }

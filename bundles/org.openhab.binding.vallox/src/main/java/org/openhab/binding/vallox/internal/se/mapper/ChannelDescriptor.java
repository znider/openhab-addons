...
public enum ChannelDescriptor {
    ...
    DC_FAN_INPUT_ADJUSTMENT  (CHANNEL_GROUP_FAN + "dcFanInputAdjustment", new IntegerChannel.DcFan(0xB0, Units.PERCENT)),
    DC_FAN_OUTPUT_ADJUSTMENT (CHANNEL_GROUP_FAN + "dcFanOutputAdjustment", new IntegerChannel.DcFan(0xB1, Units.PERCENT)),
    ...
    // Temperatures
    TEMPERATURE_INSIDE   (CHANNEL_GROUP_TEMPERATURE + "tempInside", new TemperatureChannel(0x34, SIUnits.CELSIUS)),
    TEMPERATURE_OUTSIDE  (CHANNEL_GROUP_TEMPERATURE + "tempOutside", new TemperatureChannel(0x32, SIUnits.CELSIUS)),
    TEMPERATURE_EXHAUST  (CHANNEL_GROUP_TEMPERATURE + "tempExhaust", new TemperatureChannel(0x33, SIUnits.CELSIUS)),
    TEMPERATURE_INCOMING (CHANNEL_GROUP_TEMPERATURE + "tempIncoming", new TemperatureChannel(0x35, SIUnits.CELSIUS)),
    ...
    HRC_BYPASS_THRESHOLD    (CHANNEL_GROUP_SETTINGS + "hrcBypassThreshold", new TemperatureChannel(0xAF, SIUnits.CELSIUS)),
    INPUT_FAN_STOP_THRESHOLD(CHANNEL_GROUP_SETTINGS + "inputFanStopThreshold", new TemperatureChannel(0xA8, SIUnits.CELSIUS)),
    POST_HEATING_SETPOINT   (CHANNEL_GROUP_SETTINGS + "postHeatingSetPoint", new TemperatureChannel(0xA4, SIUnits.CELSIUS)),
    PRE_HEATING_SETPOINT    (CHANNEL_GROUP_SETTINGS + "preHeatingSetPoint", new TemperatureChannel(0xA7, SIUnits.CELSIUS)),
    ...
    BASIC_HUMIDITY_LEVEL    (CHANNEL_GROUP_SETTINGS + "basicHumidityLevel", new IntegerChannel.Humidity(0xAE, Units.PERCENT)),
    ...
    // Status
    HUMIDITY            (CHANNEL_GROUP_STATUS + "humidity", new IntegerChannel.Humidity(0x2A, Units.PERCENT)),
    HUMIDITY_SENSOR_1   (CHANNEL_GROUP_STATUS + "humiditySensor1", new IntegerChannel.Humidity(0x2F, Units.PERCENT)),
    HUMIDITY_SENSOR_2   (CHANNEL_GROUP_STATUS + "humiditySensor2", new IntegerChannel.Humidity(0x30, Units.PERCENT)),
    CO2                 (CHANNEL_GROUP_STATUS + "co2", new IntegerChannel(0x00, Units.PARTS_PER_MILLION)),
    CO2_HIGH            (CHANNEL_GROUP_STATUS + "co2High", new IntegerChannel(0x2B, Units.PARTS_PER_MILLION)),
    CO2_LOW             (CHANNEL_GROUP_STATUS + "co2Low", new IntegerChannel(0x2C, Units.PARTS_PER_MILLION)),
    ...
    POST_HEATING_TARGET_VALUE(CHANNEL_GROUP_STATUS + "postHeatingTargetValue", new TemperatureChannel(0x57, SIUnits.CELSIUS)),
    ...
    SERVICE_REMINDER            (CHANNEL_GROUP_MAINTENANCE + "serviceReminderMonths", new IntegerChannel(0xA6)),
     ...
     * Get channels unit
     *
     * @return unit the channels unit
     */
    public Unit<?> getUnit() {
        Unit<?> localUnit = valloxChannel.getUnit();
        if (localUnit == null) {
            return (Unit<?>) new QuantityType<Dimensionless>();
        }
        return localUnit;
    }
